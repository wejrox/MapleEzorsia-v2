...
void dinput8::CreateHook()
{
	...
	else
	{
		...
		MessageBox(nullptr, L"Failed to load original dinput8.dll from system location, make sure your directory path is not longer than " + MAX_PATH, L"systems directory inaccessible", 0);
		ExitProcess(0);
	}
	if (const HMODULE hModule = LoadLibraryA(szPath))
	{
	...
	else
	{
		Sleep(20);
		SuspendThread(MainMain::mainThread);
		MessageBox(nullptr, L"Failed to find original dinput8.dll, verify that a non-Ezorsia v2 dinput8.dll exists in your system directory", L"Missing file", 0);
		ExitProcess(0);
	}
}
