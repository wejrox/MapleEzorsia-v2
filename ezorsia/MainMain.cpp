...
WSPPROC_TABLE MainMain::m_ProcTable = { nullptr };//to set your max level you have to go to the function rewrite
...
MainMain::MainMain(const std::function<void()>& pPostMutexFunc)
{
	std::filesystem::path baseWzFile("Base.wz");
	std::filesystem::path zmapFile("Data/zmap.img");
	std::filesystem::path configFilePath("config.ini"); //check if needed stuff exists
	if (exists(configFilePath) && reader.ParseError())
	{
		Sleep(20);
		SuspendThread(mainThread);
		MessageBox(nullptr, L"your config.ini file cannot be properly read, go to troubleshooting section of Ezorsia v2 setup guide at https://github.com/444Ro666/MapleEzorsia-v2 for more details, or delete your config.ini to have a new one generated with default settings", L"bad config file", 0);
		ExitProcess(0);
	}
	if (!exists(configFilePath))
	{
		HANDLE hConfigFile = CreateFileA("config.ini", (GENERIC_READ | GENERIC_WRITE), NULL, nullptr, CREATE_ALWAYS, NULL, nullptr);
		DWORD dwBytesWritten;
		if (hConfigFile)
		{
			HMODULE hDll = GetModuleHandle(L"dinput8.dll"); // Get handle to current DLL
			HRSRC hrDll = FindResource(hDll, MAKEINTRESOURCE(IDR_RCDATA1), RT_RCDATA);
			HGLOBAL rdDll = LoadResource(hDll, hrDll);
			DWORD dwResourceSize = SizeofResource(hDll, hrDll);
			LPVOID lockedResourceData = LockResource(rdDll);
			Sleep(2);
			SuspendThread(mainThread);
			if (WriteFile(hConfigFile, lockedResourceData, dwResourceSize, &dwBytesWritten, nullptr) && dwBytesWritten == dwResourceSize)
			{
	...
	if (!exists(baseWzFile) && !exists(zmapFile))
	{
		Sleep(20);
		SuspendThread(mainThread);
		MessageBox(nullptr, L"Either Base.wz is missing from your game directory OR you are loading from .img and zmap.img is not in your Data directory, please reinstall and make sure relevant file(s) exist", L"missing .wz/.img file", 0);
		ExitProcess(0);
	}
	CustomLoginFrame = reader.GetBoolean("optional", "CustomLoginFrame", false);
	if (CustomLoginFrame)
	{
		ownLoginFrame = true;
		bigLoginFrame = true;
	...
	if (exists(EfilePath))
	{ 	//only check after "if false" on Client::CustomLoginFrame or things break
		EzorsiaV2WzIncluded = true;
		CustomLoginFrame = true;
		usingEzorsiaV2Wz = true;
	}
	else if (exists(EfilePath2))
	{
		EzorsiaV2WzIncluded = true;
		CustomLoginFrame = true;
	}
	...
	{
		if (exists(baseWzFile))
		{
			HANDLE hOrg = CreateFileA("EzorsiaV2_UI.wz", (GENERIC_READ | GENERIC_WRITE), NULL, nullptr, CREATE_ALWAYS, NULL, nullptr);
			DWORD dw;
			if (hOrg)
			{
				...
				Sleep(2);
				SuspendThread(mainThread);
				if (WriteFile(hOrg, resourceData, resourceSize, &dw, nullptr) && dw == resourceSize)
				{
					CloseHandle(hOrg);
					ResumeThread(mainThread);
				}
				...
				{
					CloseHandle(hOrg);
					MessageBox(nullptr, L"your EzorsiaV2_UI.wz file doesn't exist, please re-download EzorsiaV2_UI.wz from Ezorsia v2 releases at https://github.com/444Ro666/MapleEzorsia-v2", L"bad WZ file", 0);
					ExitProcess(0);
				}
			}
			EzorsiaV2WzIncluded = true;
		...
		if (exists(zmapFile))
		{
			HANDLE hOrg = CreateFileA("Data\\MapleEzorsiaV2wzfiles.img", (GENERIC_READ | GENERIC_WRITE), NULL, nullptr, CREATE_ALWAYS, NULL, nullptr);
			DWORD dw;
			if (hOrg)
			{
				HMODULE hModule = GetModuleHandle(L"dinput8.dll"); // Get handle to current DLL
				...
				{
					CloseHandle(hOrg);
					MessageBox(nullptr, L"Ezorsia V2 has detected you are loading from .img, but that your MapleEzorsiaV2wzfiles.img file doesn't exist in your Data folder, please re-download MapleEzorsiaV2wzfiles.img from Ezorsia v2 releases at https://github.com/444Ro666/MapleEzorsia-v2", L"bad IMG file", 0);
					ExitProcess(0);
				}
			}
			EzorsiaV2WzIncluded = true;
	...
	useV62_ExpTable = reader.GetBoolean("optional", "useV62_ExpTable", false);
	const char* serverIP_Address = Client::ServerIP_AddressFromINI.c_str();
	m_sRedirectIP = serverIP_Address;
	...
	const char* use_custom_dll_1c = use_custom_dll_1s.c_str();
	const char* use_custom_dll_2c = use_custom_dll_2s.c_str();
	const char* use_custom_dll_3c = use_custom_dll_3s.c_str();
	use_custom_dll_1 = use_custom_dll_1c;
	use_custom_dll_2 = use_custom_dll_2c;
	use_custom_dll_3 = use_custom_dll_3c;
...
//fast so favor lower numbers. if the number is too high the non-system hooks wont reach the main thread in time and the main thread will
	...
	if (strcmp(use_custom_dll_1, "CUSTOM.dll"))
	{ //load auxilliary dll files
		HMODULE hModule = LoadLibraryA(use_custom_dll_1);
		if (!hModule)
		{
			Sleep(20);
			SuspendThread(mainThread);
			MessageBox(nullptr, L"Failed to find the first custom dll file", L"Missing file", 0);
			ExitProcess(0);
		}
	}
	if (strcmp(use_custom_dll_2, "CUSTOM2.dll"))
	{
		HMODULE hModule1 = LoadLibraryA(use_custom_dll_2);
		if (!hModule1)
		{
			Sleep(20);
			SuspendThread(mainThread);
			MessageBox(nullptr, L"Failed to find the second custom dll file", L"Missing file", 0);
			ExitProcess(0);
		}
	}
	if (strcmp(use_custom_dll_3, "CUSTOM3.dll"))
	{
		HMODULE hModule2 = LoadLibraryA(use_custom_dll_3);
		if (!hModule2)
		{
			Sleep(20);
			SuspendThread(mainThread);
			MessageBox(nullptr, L"Failed to find the third custom dll file", L"Missing file", 0);
			ExitProcess(0);
		}
	}
