...
{
	constexpr size_t nSize = sizeof(ucValue);
	if (UseVirtuProtect)
	{
...
{ //tested and working
	__try
	{
		...
		WriteInt(dwOriginAddress + 1, (int)ptrCodeCave - static_cast<int>(dwOriginAddress) - 5); // [jmp(1 byte)][address(4 bytes)] //this means you need to clear a space of at least 5 bytes (nNOPCount bytes)
